                        ...
                        configurationSpace.isTraversable(nSucc))
                    {
                        ...
                        if (!nodes3D[iSucc].isClosed() || iPred == iSucc)
                        {
                            ...
                            if (!nodes3D[iSucc].isOpen() ||
                                newG < nodes3D[iSucc].getG() || iPred == iSucc)
                            {
                                // calculate H value
             ...
             CollisionDetection& configurationSpace, Visualize& visualization)
{
    ...
    if (Constants::dubins)
    {
        ...
        //    // if the lookup table flag is set and the vehicle is in the
        //    lookup area if (Constants::dubinsLookup && uX <
        //    Constants::dubinsWidth - 1 && uY < Constants::dubinsWidth - 1) {
        ...
        /*if (Constants::dubinsShot && std::abs(start.getX() - goal.getX()) >=
         * 10 && std::abs(start.getY() - goal.getY()) >= 10)*/
        //      // start
    ...
    int index = (int)start.getY() * width + (int)start.getX();
    if (Constants::twoD && !nodes2D[index].isDiscovered())
    {
        //    ros::Time t0 = ros::Time::now();
        ...
        nodes2D[index].setG(aStar(goal2d, start2d, nodes2D, width, height,
                                  configurationSpace, visualization));
        //    ros::Time t1 = ros::Time::now();
        //    ros::Duration d(t1 - t0);
    ...
    if (Constants::twoD)
    {
        ...
        float x_offset = (start.getX() - (long)start.getX()) -
                         (goal.getX() - (long)goal.getX());
        float y_offset = (start.getY() - (long)start.getY()) -
                         (goal.getY() - (long)goal.getY());
        twoDoffset = sqrt(powf(x_offset, 2) + powf(y_offset, 2));
        twoDCost = nodes2D[index].getG() - twoDoffset;
    }
